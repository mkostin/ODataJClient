...
public class PropertyUpdateITCase extends AbstractTest{
	...
	public void updateComplexProperty(
        ...
        final ODataPropertyUpdateRequest req = ODataCUDRequestFactory.getPropertyComplexValueUpdateRequest(uriBuilder.build(), type, dimensionToBeUpdated);
        ...
        if(prefer.equals("return-content")){
        	...
        	System.out.println(property.hasComplexValue());
        	ODataComplexValue value = property.getComplexValue();
        	System.out.println(value.get("Depth").getValue().toString());
	...
	public void updateCollectionProperty(
			...
			final String propertyType){
		final ODataURIBuilder uriBuilder = new ODataURIBuilder(testDefaultServiceRootURL).
                appendEntityTypeSegment(entity).appendKeySegment(-9). appendStructuralSegment("PrimaryContactInfo").appendStructuralSegment(propertyType);
	...
	public void updateStringPropertyValue(
		 ...
		 final ODataURIBuilder uriBuilder = new ODataURIBuilder(testDefaultServiceRootURL).
	        ...
	        System.out.println(newVal+"----new val");
	...
	public static void updateDatePropertyValue(
		...
		try{
		 final ODataURIBuilder uriBuilder = new ODataURIBuilder(testDefaultServiceRootURL).
	        ...
	        retrieveReq.setPrefer(prefer);
	        ODataRetrieveResponse<ODataValue> retrieveRes = retrieveReq.execute();
	        System.out.println(retrieveRes.getStatusCode());
	        assertEquals(200, retrieveRes.getStatusCode());
	        String etag = retrieveRes.getEtag();
	        ...
	        assertNotNull(date);
	        System.out.println(date.asPrimitive().<String>toCastValue());
	...
	public static void updateIntegerPropertyValue(
		...
		try{
		 ...
		 final ODataURIBuilder uriBuilder = new ODataURIBuilder(testDefaultServiceRootURL).
	        ...
	        final ODataPrimitiveValue newVal = new ODataPrimitiveValue.Builder().setValue(newItem).setType(EdmSimpleType.INT_32).build();
	        System.out.println(newVal+"----new val");
	        final ODataValueUpdateRequest updateReq =
	                ODataCUDRequestFactory.getValueUpdateRequest(uriBuilder.build(), type, newVal);
	        updateReq.setFormat(ODataValueFormat.TEXT);
	...
	public void updateBooleanPropertyValue(
		...
		final ODataURIBuilder uriBuilder = new ODataURIBuilder(testDefaultServiceRootURL).
	        ...
	        System.out.println("Old value "+oldItem);
	        final ODataPrimitiveValue newVal = new ODataPrimitiveValue.Builder().setValue(!oldItem).setType(EdmSimpleType.BOOLEAN).build();
